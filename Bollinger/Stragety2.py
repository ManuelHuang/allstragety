...
class TestStrategy2(bt.Strategy):
    ...
    def __init__(self,trail_percentage_sell=0.04,trail_percentage_buy=0.05,back_stop_order_percentage_sell=1.04,back_stop_order_percentage_buy=1.03,period_buy=1800,period_sell=2160,devfactors_buy = 3,devfactors_sell=3):
        self.dataclose = self.datas[0].close
        self.dataopen = self.datas[0].open
        self.datahigh = self.datas[0].high
        self.datalow = self.datas[0].low
        self.trail_percentage_sell = trail_percentage_sell #0.04
        self.trail_percentage_buy = trail_percentage_buy #0.05
        self.back_stop_order_percentage_sell = back_stop_order_percentage_sell #测试范围 1.005-1.02，步长0.001,1.04
        self.back_stop_order_percentage_buy= back_stop_order_percentage_buy #1.03
        self.bolling = bt.indicators.BBands(period = period_buy, devfactor = devfactors_buy) #period测试范围 1440-2880 步长120  devfactor 测试范围 3-7 步长0.5 1800,3
        self.bolling_sell = bt.indicators.BBands(period = period_sell, devfactor = devfactors_sell) #2160,3
    ...
    def next(self):
        ...
        if self.position.size == 0 and self.datas[0].datetime.time().hour == 00 and len(self) > 1800:
            ...
            if self.is_sell:
                self.order_sell = self.sell(exectype=bt.Order.Stop, price= self.bolling_sell.bot,
                                            size=self.trade_amount)

...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self,period_me1_buy=840,period_me1_sell=120,period_me2_buy=1620,period_me2_sell=180,period_signal_buy=780,period_signal_sell=60,period_sleep_buy= 240,period_sleep_sell= 120,back_stop_order_percentage_buy = 1.04,back_stop_order_percentage_sell = 1.03):
        ...
        if self.position.size == 0 and len(self) > self.period :
            ...
            if self.is_sell:
                if self.macd_sell.macd <0 and self.macd_sell.signal <0:
                    self.order_sell = self.buy(size =self.trade_amount)

        elif self.position.size < 0:
            self.order_sell = None

            if self.order_sell_stop_back_up is None:
                self.order_sell_stop_back_up = self.buy(size=abs(self.position.size), exectype=bt.Order.Stop,price=self.position.price * self.back_stop_order_percentage_sell)
            ...
            if not self.order_buy_stop_back_up:
                self.order_buy_stop_back_up = self.sell(size=abs(self.position.size), exectype=bt.Order.Stop, price=self.position.price / self.back_stop_order_percentage_buy)
